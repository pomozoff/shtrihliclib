...
using handles_t = std::map<const std::shared_ptr<const CheckMethodLogin>, const handle_t>;
using check_methods_t = std::vector<std::shared_ptr<const CheckMethod>>;
...
class KeyChecker : public IKeyChecker, public ICheckMethodsManager, public ICheckMethodsViewer {
	...
	protected:
		mutable check_methods_t m_check_methods;
